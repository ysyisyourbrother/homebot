...
_TOOL_RESULTS_DIR = ".homebot/tool-results"
...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    """Create the guest profile and workspace directories when missing."""
    from importlib.resources import files as pkg_files

    try:
        templates = pkg_files("homebot") / "templates"
    except Exception:
        return []

    added: list[str] = []
    guest_template = templates / "members" / "guest" / "USER.md"
    guest_profile = workspace / "members" / "guest" / "USER.md"
    if guest_template.is_file() and not guest_profile.exists():
        guest_profile.parent.mkdir(parents=True, exist_ok=True)
        guest_profile.write_text(guest_template.read_text(encoding="utf-8"), encoding="utf-8")
        added.append("members/guest/USER.md")
