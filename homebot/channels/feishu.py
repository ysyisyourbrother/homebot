...
class _LoopProxy:
    """Thread-local proxy for ``lark_oapi.ws.client.loop``.

    lark SDK 的 ws Client.start() 通过模块级 ``loop`` 变量操作事件循环。
    多 FeishuChannel 实例各自在独立线程跑 WS 时，若直接赋值会互相覆盖。
    此代理将 loop 存储为线程本地变量，每个线程读写自己的事件循环，消除竞争。
    """

    def __init__(self, default_loop):
        self._default = default_loop
    ...
    @property
    def _loop(self):
        return getattr(self._loops, "loop", self._default)
...
class FeishuChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        # 将 lark SDK 的模块级 loop 替换为线程本地代理（仅首次）。
        # 之后各 WS 线程通过 proxy.set(loop) 注册自己的事件循环，消除竞争。
        import lark_oapi.ws.client as _lark_ws_client

        if not isinstance(_lark_ws_client.loop, _LoopProxy):
            _lark_ws_client.loop = _LoopProxy(_lark_ws_client.loop)
        ...
        def run_ws():
            ...
            # 线程本地的 loop 代理，各线程读写互不干扰
            _lark_ws_client.loop.set(ws_loop)
