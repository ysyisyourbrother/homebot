...
from homebot.utils.restart import consume_restart_notice_from_env, format_restart_completed_message
...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        for name, cls in discover_all().items():
            ...
            try:
                ...
                channel._session_cleanup = (
                    self._session_manager.delete_session if self._session_manager else None
                )
        ...
        # 多实例通道支持：检测配置中的 feishu_xxx 等派生键，
        # 为每个创建一个独立的通道实例。
        # 注意：Pydantic extra 字段不在 dir() 中，需用 model_extra。
        extra = getattr(self.config.channels, "model_extra", None) or {}
        for base_name, cls in discover_all().items():
            for key, section in extra.items():
                if not key.startswith(f"{base_name}_"):
                    continue
                ...
                try:
                    ...
                    logger.info("{} channel enabled", key)
                except Exception as e:
    ...
    def _resolve_transcription_key(self, provider: str) -> str:
        ...
        provider_cfg = getattr(self.config.providers, provider, None)
        if provider_cfg is None:
            return ""
        return getattr(provider_cfg, "api_key", "") or ""
    ...
    def _resolve_transcription_base(self, provider: str) -> str:
        ...
        if provider_cfg is None:
            return ""
        return getattr(provider_cfg, "api_base", "") or ""
    ...
    async def cancel_active_interactions(self) -> None:
        """Reset local channels after an Esc cancellation."""
        for channel in self.channels.values():
            cancel = getattr(channel, "cancel_interaction", None)
            if cancel:
                await cancel()
