...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/HKUDS/homebot",
    "X-OpenRouter-Title": "homebot",
...
def _uses_openrouter_attribution(spec: "ProviderSpec | None", api_base: str | None) -> bool:
    """Apply Homebot attribution headers to OpenRouter requests by default."""
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _build_kwargs(
        ...
        # Always explicitly set when the provider supports thinking, so
        # the API behaviour is deterministic regardless of model defaults
        # (e.g. DeepSeek V4 Flash defaults thinking to enabled).
        ...
        thinking_enabled = (
            ...
            and semantic_effort is not None
        ...
        if response_format and spec and spec.name == "deepseek":
        ...
        # When thinking is active, backfill reasoning_content on assistant
        # messages that lack it — DeepSeek V4 rejects requests where some
        # assistant messages are missing the field.
        # When thinking is explicitly disabled, strip any residual
        # reasoning_content from session history so the API does not
        # mistakenly treat the request as thinking-mode.
        if spec and spec.thinking_style:
    ...
    async def chat_stream(
        ...
        idle_timeout_s = int(os.environ.get("HOMEBOT_STREAM_IDLE_TIMEOUT_S", "90"))
        try:
            kwargs = self._build_kwargs(
                messages, tools, model, max_tokens, temperature,
                reasoning_effort, tool_choice, response_format,
