...
class LLMProvider(ABC):
    ...
    def _tool_cache_marker_indices(cls, tools: list[dict[str, Any]]) -> list[int]:
        """Return cache marker indices: tail index for tool block boundary."""
        if not tools:
            return []
        return [len(tools) - 1]
        ...
        *,
        ...
        for attempt in range(len(delays) + 1):
            ...
            if attempt >= len(delays):
                logger.warning("LLM request failed after {} retries, giving up: {}",
                    attempt + 1, (response.content or "")[:120].lower())
                break

            delay = delays[attempt]
            logger.warning("LLM transient error (attempt {}/{}), retrying in {}s: {}",
                attempt + 1, len(delays), delay, (response.content or "")[:120].lower())
