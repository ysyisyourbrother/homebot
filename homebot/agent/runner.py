...
from homebot.providers.base import LLMProvider, ToolCallRequest
from homebot.utils.helpers import (
...
from homebot.utils.runtime import (
...
class AgentRunner:
    ...
    async def run(self, spec: AgentRunSpec) -> AgentRunResult:
        ...
        injection_cycles = 0
        deferred_stream_deltas: list[str] = []

        iteration = 0
        for iteration in range(spec.max_iterations):
            try:
                # Snip history to fit context window, then clean up any orphan
                # tool results that were separated from their assistant messages.
                messages_for_model = self._snip_history(spec, messages)
                messages_for_model = self._drop_orphan_tool_results(messages_for_model)
            except Exception as exc:
                logger.warning(
                    "Context governance failed on turn {} for {}: {}",
                    iteration,
            ...
            if response.should_execute_tools:
                ...
                empty_content_retries = 0
                length_recovery_count = 0
                # Drain injections after tools, before next LLM call
    ...
    async def _request_model(
        ...
        if hook.wants_streaming():
            async def _stream(delta: str) -> None:
                if spec.defer_stream_until_no_tools:
                    deferred_stream_deltas.append(delta)
                else:
                    await hook.on_stream(context, delta)
